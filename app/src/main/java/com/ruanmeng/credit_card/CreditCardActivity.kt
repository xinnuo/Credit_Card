        ...
        when (v.id) {
            ...
            R.id.back_hint -> {
                DialogHelper.showDialog(
                        baseContext,
                        "温馨提示",
                        "Card Vеrification Number，卡确认码/安全码。这是银行卡用于非直接场合，如网络支付等下交易使用的。可以识别银行卡交易的在场性。一般可见于银行卡背面的签名条一串数列的末三位。对于银联组织的银联标准卡使用的称为CVN2，万事达卡称为CVC2，VISA卡使用的称为CVV2，AE运通卡则称为CSC2。",
                        "确定")
            }
            R.id.date_hint -> {
                DialogHelper.showDialog(
                        baseContext,
                        "温馨提示",
                        "信用卡的有效期以“月月/年年”表示，如：10/12表示有效期为2012年10月31日。银行信用卡有效期一般为3年，个别银行为5年。有效期到期后，银行将根据以往客户使用信用卡的情况，决定是否让客户继续使用信用卡。",
                        "确定")
            }
            R.id.bt_yzm -> {
